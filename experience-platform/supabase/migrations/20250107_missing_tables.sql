...
-- Content posts table (blog, news, press releases)
create table if not exists content_posts (
  ...
  post_type text not null default 'article', -- article, news, press_release, guide
...
-- User event schedules (personal schedule builder)
create table if not exists user_event_schedules (
  id uuid primary key default uuid_generate_v4(),
  user_id uuid references auth.users(id) on delete cascade,
  event_id uuid references events(id) on delete cascade,
  schedule_items jsonb not null, -- array of {artist_id, stage_id, start_time, notes}
...
-- Brand integrations (third-party integration configs)
create table if not exists brand_integrations (
  id uuid primary key default uuid_generate_v4(),
  brand_id uuid references brands(id) on delete cascade,
  integration_type text not null, -- spotify, youtube, shopify, twilio, etc.
