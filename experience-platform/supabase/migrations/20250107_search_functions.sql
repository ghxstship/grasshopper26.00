...
CREATE INDEX IF NOT EXISTS idx_artists_search 
ON artists USING gin(to_tsvector('english', name || ' ' || COALESCE(bio, '') || ' ' || array_to_string(genre_tags, ' ')));

CREATE INDEX IF NOT EXISTS idx_products_search 
ON products USING gin(to_tsvector('english', name || ' ' || COALESCE(description, '') || ' ' || COALESCE(category, '')));

CREATE INDEX IF NOT EXISTS idx_content_posts_search 
ON content_posts USING gin(to_tsvector('english', title || ' ' || COALESCE(content, '') || ' ' || array_to_string(tags, ' ')));
