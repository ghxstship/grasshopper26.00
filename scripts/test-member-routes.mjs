 ...
 * Tests both page routes and API endpoints
 */
...
async function runTests() {
  ...
  for (const route of ROUTES) {
    process.stdout.write(`Testing: ${route.name.padEnd(40)} `);
...
async function main() {
  try {
    ...
    await new Promise(resolve => setTimeout(resolve, 3000)); // Give server time to fully initialize
    await runTests();
    printSummary();
  } catch (error) {
