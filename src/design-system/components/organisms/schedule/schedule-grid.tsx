...
export function ScheduleGrid({ eventId, className }: ScheduleGridProps) {
  ...
  if (loading) {
    return <div className={styles.loadingContainer}>Loading schedule...</div>;
  }
      ...
      {days.length > 1 && (
        <Tabs value={selectedDay} onValueChange={setSelectedDay}>
          <TabsList className={styles.fullWidth}>
            {days.map((day) => (
              <TabsTrigger key={day} value={day} className={styles.row}>
      ...
      {mySchedule.size > 0 && (
        <Card className={styles.summaryCard}>
          <div className={styles.summaryContent}>
            <div>
              <h3 className={styles.summaryTitle}>My Schedule</h3>
...
function ScheduleCard({
  ...
  return (
    <Card className={cn(
      styles.scheduleCard,
      isInMySchedule && styles.scheduleCardSelected,
      hasConflict && styles.scheduleCardConflict
    )}>
      <div className={styles.cardContent}>
        <div className={styles.cardHeader}>
          <div className={styles.cardInfo}>
            <h3 className={styles.artistName}>{item.artist_name}</h3>
            <div className={styles.stageInfo}>
              <MapPin className={styles.container} />
        ...
        <div className={styles.timeInfo}>
          <Clock className={styles.container} />
          <span>
        ...
        {item.special_notes && (
          <p className={styles.specialNotes}>{item.special_notes}</p>
        )}

        {hasConflict && (
          <Badge variant="destructive" className={styles.text}>
            Schedule Conflict
          </Badge>
        )}
...
function ScheduleListItem({
  ...
  return (
    <Card className={cn(
      styles.listItem,
      isInMySchedule && styles.listItemSelected,
      hasConflict && styles.listItemConflict
    )}>
      <div className={styles.listItemContent}>
        <div className={styles.listItemInfo}>
          ...
          <div className={styles.listItemDetails}>
            <div className={styles.listItemArtist}>{item.artist_name}</div>
            <div className={styles.listItemStage}>{item.stage_name}</div>
          </div>
          {hasConflict && (
            <Badge variant="destructive" className={styles.text}>
              Conflict
